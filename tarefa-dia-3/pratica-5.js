...
let tamanho = receita.length;
let i = 0;
let valorSemGorjeta = 0;


while (i < tamanho) {
    valorSemGorjeta = valorSemGorjeta + receita[i].valor;
    i++
}

console.log(`O valor total de vendas sem gorjeta foi de ${valorSemGorjeta}`);

/* ----------------------------------------------------------------------------- */

/* PROBLEMA 2: Qual o valor total de vendas incluindo a gorjeta? --------------- */
//COLOQUE SUA SOLUÇÃO AQUI
let valorComGorjeta = 0;
i = 0;

while (i < tamanho) {
    valorComGorjeta = valorComGorjeta + receita[i].gorjeta + receita[i].valor;
   // console.log(`${valorComGorjeta}`)
   // console.log(`${receita[i].gorjeta}`)
    i++;
}

console.log(`O valor total de vendas com gorjeta foi de ${valorComGorjeta}`);
...
for (let i in receita) {
    if ((receita[i].valor + receita[i].gorjeta) <= 50) {
    contador++;
    }
    else {
    }
...
console.log(`O numero de vendas abaixo de 50 reais foi de ${contador}`)
